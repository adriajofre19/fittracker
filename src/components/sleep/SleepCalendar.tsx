...
export default function SleepCalendar({
    ...
    const [markedDates, setMarkedDates] = useState<Map<string, SleepRecord>>(new Map());

    useEffect(() => {
        const datesMap = new Map<string, SleepRecord>();
        sleepRecords.forEach((record) => {
            datesMap.set(record.sleep_date, record);
        });
    ...
    const getRecordForDate = (date: Date): SleepRecord | undefined => {
                ...
                <CardContent className="p-4 sm:p-6 flex-1 flex flex-col">
                    ...
                    <div className="flex-1 grid grid-cols-7 gap-1 auto-rows-fr">
                        {daysInMonth.map((date, index) => {
                            ...
                            const qualityColor = record ? getSleepQualityColor(record.total_sleep_hours) : "";
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {record && (
                                        <div className={`absolute bottom-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full ${qualityColor}`} />
                                    )}
                                </button>
                            );
                        })}
